...
"""
Script to identify the most frequently mentioned
entities in the wikified version of the Speculum
archives. Plotted categories include e.g. authors.
"""
...
tcs = {'authors': {'person', 'writer'},
       'poems': {'poem'},
       'saints': {'saint'}}

for n, target_categories in tcs.items():
    ...
    for entity in full_vocab:
        try:
            ...
            if target_categories:
                if atts and (target_categories.intersection(atts) ==
                             target_categories):
                    vocab.append(entity)
            else:
                vocab.append(entity)
    ...
    fig, ax1 = sb.plt.subplots(figsize=(14, 7))
    ...
    for cum, cv, label in zip(cumul, cvs, labels):
        ...
        ax1.text(cv, cum, label, ha='center', va="center",
                 fontdict={'family': 'Arial', 'size': (12 / cv),
                           'color': 'darkslategrey'})

    ax1.set_title('Most frequently wikified ' + n + ' in Speculum')
    ax1.set_xlabel('Coefficient of Variation (log)')
    ax1.set_ylabel('Cumulative Frequency (log)')
    sb.plt.savefig('../figures/05' + n + '_mentioned.pdf', bbox_inches=0)
    sb.plt.close()
